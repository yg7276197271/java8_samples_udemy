...
public class Set11_CodeDecode {
	...
	public static void main(String[] args) {
		...
		System.out.println("-----------------------------------------");
		//removeDuplicateElementsinIntegerList();
	}
	...
	private static void countNoOfOccurenacesOfWordinString() {
		...
		list.stream().collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))//This will return map
		.entrySet()
		.stream()
	...
	private static void removeDuplicateElementsinIntegerList() {
		List<Integer> list = Arrays.asList(10, 28, 87, 10, 76, 28, 26, 80, 80);

		Set<Integer> duplicatesRemoved = new HashSet<Integer>();
		list.stream().filter(e -> duplicatesRemoved.add(e)).forEach(x -> System.out.println(x));
	}
