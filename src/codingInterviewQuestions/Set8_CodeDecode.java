...
public class Set8_CodeDecode {
	...
	private static void sortEmployeeWithSalary(List<Emp> empList) {
		...
		List<Emp> empSortedListUsingComparing = empList.stream().sorted(Collections.reverseOrder(
				Comparator.comparing(Emp::getSalary)))
				.collect(Collectors.toList());
		System.out.println("empSortedList in descending order of salary " + empSortedListUsingComparing);
		...
		List<Emp> empSortedList1 = empList.stream().sorted(Collections.reverseOrder((e1, e2) ->
