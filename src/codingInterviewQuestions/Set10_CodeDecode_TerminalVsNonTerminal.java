...
public class Set10_CodeDecode_TerminalVsNonTerminal {
	...
	public static void main(String[] args) {
		...
		empList.stream().filter(e -> e.getId()%2 == 0).map(e-> {
			e.printName();
			return e.getName();
		}).forEach(e -> System.out.println("At Last "+e)); //forEach is terminal operation

		empList.stream().filter(e -> e.getId()%2 == 0).map(e-> {e.printName();return e.getName();}); 
		//filter , map are intermediate operations which are called only when there is terminal op
...
/*		System.out.println(intList.stream().filter(a -> a%2 == 0)
		...
		System.out.println("Sum of elements using reduce with Optional "+intList.stream().reduce((a,b)-> a+b).get());
		System.out.println("Sum of elements using reduce without Optional "+intList.stream().reduce(0,(a,b)-> a+b));
