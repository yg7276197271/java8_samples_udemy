...
public class Set2_CodeDecode {
	...
	// Here we have used toSet() as second arg -> how the grouped elements to be collected
	...
	private static void groupEmployeesByAgeAndUniqueEmployeeAndSorted() {
		...
		empMap.entrySet().forEach(set -> System.out.println("Key is "+set.getKey() +"Value is "+set.getValue()));
	}
	
	//Internally single parameter groupingBy calls two parameter
	//groupingBy(classifier,toList())
	
	//Internally 2 parameters grouping by calls three parameters
	//groupingBy(classifier, HashMap::new,toList())

	//Convert Employee List to List of names then convert it to upperCase
	//And we want to join all the names using , separator
